...
<div class="py-1 w-full mx-auto">
    <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm p-4">
        ...
        <div id="posts-table-wrap" class="overflow-x-auto">
            <table class="w-full table-fixed text-left border-collapse border-x border-slate-200 dark:border-slate-700">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                    <tr class="border-y border-slate-200 dark:border-slate-700 divide-x divide-slate-200 dark:divide-slate-700">
                        @if(auth()->user()->canFeature('posts.manage'))
                        <th class="bulk-select-col hidden py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 w-12 text-center">
                        ...
                        @endif
                        <th class="py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 w-16 text-center">SL</th>
                        <th class="py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 min-w-0">Title</th>
                        <th class="py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 w-24 text-center">Image</th>
                        <th class="py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 w-24">Categories</th>
                        <th class="py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 w-24">Status</th>
                        <th class="py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 w-24">Date/Time</th>
                        <th class="py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 w-32">Reporter</th>
                        <th class="py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 w-28">Published By</th>
                        <th class="py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 w-28">Edited By</th>
                        <th class="post-action-col py-2 px-2 text-xs font-semibold text-slate-900 dark:text-slate-100 text-right w-20">Action</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    ...
                    <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors divide-x divide-slate-200 dark:divide-slate-700">
                        @if(auth()->user()->canFeature('posts.manage'))
                        <td class="bulk-select-col hidden py-2 px-2 text-center">
                        ...
                        <td class="py-2 px-2 min-w-0">
                            <div class="text-sm font-normal text-slate-900 dark:text-slate-100 line-clamp-2 group-hover:text-indigo-600 transition-colors break-words">
                                {{ $post->title }}
                            </div>
                        </td>
                        <td class="py-2 px-2 text-center">
                        ...
                        <td class="py-2 px-2">
                            ...
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-normal bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-500/20">
                                Published
                            </span>
                        ...
                        <td class="py-2 px-2">
                            <span class="text-xs font-normal text-slate-600 dark:text-slate-300">{{ person_name_first_two_words(optional($post->creator)->name) ?? 'N/A' }}</span>
                        </td>
                        <td class="py-2 px-2">
                            <span class="text-xs font-normal text-slate-600 dark:text-slate-300">{{ person_name_first_two_words(optional($post->editor)->name) ?? '—' }}</span>
                        </td>
                        <td class="py-2 px-2 text-right post-action-col">
                    ...
                    @empty
                    <tr>
                        <td colspan="{{ auth()->user()->canFeature('posts.manage') ? '11' : '10' }}" class="py-12 text-center text-slate-400 text-sm">No posts found. <a href="{{ route('admin.posts.create') }}" class="text-indigo-500 hover:underline">Create your first post.</a></td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<script>
    (function() {
        ...
        function prepareBulkForm() {
            if (!bulkForm) return;
            bulkForm.querySelectorAll('input.bulk-hidden-id').forEach(function(el) {
                el.remove();
            });
        ...
        function updateBulkState() {
            ...
            var checkedOnPage = boxes.filter(function(cb) {
                return cb.checked;
            });
