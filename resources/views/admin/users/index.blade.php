...
<div class="py-1 w-full mx-auto">
    <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm p-4">
        <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-between pb-6 border-b border-slate-100 dark:border-slate-800 mb-6 sm:mb-8">
            <form method="GET" action="{{ route('admin.users.index') }}" class="relative w-full min-w-0 max-w-full sm:max-w-sm flex items-center">
                <input
                    type="text"
                    name="search"
                    value="{{ request('search') }}"
                    placeholder="Search user (SL, name, email, phone)..."
                    class="w-full pl-10 pr-10 py-2 rounded-lg border border-slate-200 dark:border-slate-800 focus:ring-1 focus:ring-indigo-500 transition-all outline-none font-normal text-slate-900 text-sm"
                >
        ...
        @if(session('success'))
            <div class="mb-4 px-4 py-2.5 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 rounded-lg text-sm text-emerald-700 dark:text-emerald-400">{{ session('success') }}</div>
        @endif
        @if(session('error'))
            <div class="mb-4 px-4 py-2.5 bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20 rounded-lg text-sm text-rose-700 dark:text-rose-400">{{ session('error') }}</div>
        @endif
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    ...
                    <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors border-b border-slate-200 dark:border-slate-700 divide-x divide-slate-200 dark:divide-slate-700">
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-slate-100 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 overflow-hidden shrink-0">
                                    @if($user->image)
                                        <img src="{{ storage_image_url($user->image) }}" class="w-full h-full object-cover" alt="">
                                    @else
                        ...
                        <td class="py-3 px-4">
                            ...
                            @elseif($user->role == 'sub editor' || $user->role == 'reporter')
                        ...
                        <td class="py-3 px-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @php
                                    $authUser = auth()->user();
                                    $canDeleteUser = ($authUser->role === 'admin' && $authUser->id !== $user->id && in_array($user->role, ['senior editor', 'sub editor'], true))
                                        || ($authUser->role === 'senior editor' && $user->role === 'sub editor');
                                @endphp
                                @if($canDeleteUser)
