...
@section('header_title', 'All Galleries')
...
<div class="py-1 w-full mx-auto">
    <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm p-4">
        <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-between pb-4 border-b border-slate-100 dark:border-slate-800 mb-4">
            <form method="GET" action="{{ route('admin.galleries.index') }}" class="relative w-full min-w-0 max-w-full sm:max-w-sm flex items-center">
                <input
                    type="text"
                    name="search"
                    value="{{ request('search') }}"
                    placeholder="Search galleries (SL or title)..."
                    class="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 dark:border-slate-800 focus:ring-1 focus:ring-indigo-500 transition-all outline-none font-normal text-slate-900 text-sm"
                >
            ...
            <a href="{{ route('admin.galleries.create') }}" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-normal rounded-lg transition-all shadow-md text-sm shrink-0">
                Add New Gallery
            </a>
        </div>
            ...
            <table class="w-full text-left border-collapse border-x border-slate-200 dark:border-slate-700">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                    <tr class="border-y border-slate-200 dark:border-slate-700 divide-x divide-slate-200 dark:divide-slate-700">
                        ...
                        <th class="py-3 px-4 text-xs font-semibold text-slate-900 dark:text-slate-100">Cover</th>
                        <th class="py-3 px-4 text-xs font-semibold text-slate-900 dark:text-slate-100">Title & Description</th>
                    ...
                    <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors divide-x divide-slate-200 dark:divide-slate-700">
                        ...
                        <td class="py-3 px-4">
                            <div class="h-10 w-14 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden bg-slate-50 flex items-center justify-center">
                                @if($firstImage)
                                    <img src="{{ storage_image_url($firstImage->image) }}" class="h-full w-full object-cover" alt="Gallery Cover">
                                @else
                        ...
                        <td class="py-3 px-4 max-w-xs">
                            <div class="text-sm font-normal text-black dark:text-white group-hover:text-indigo-600 transition-colors">{{ $gallery->title }}</div>
                            @if($gallery->description)
                                <div class="text-xs text-slate-400 mt-0.5 truncate max-w-[220px]">{{ $gallery->description }}</div>
                            @endif
                        </td>
                        <td class="py-3 px-4">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-500/20">
                                {{ optional($gallery->category)->name ?? 'None' }}
                            </span>
                        </td>
                        <td class="py-3 px-4 text-center">
                            <span class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ $gallery->images->count() }}</span>
                            <span class="text-xs text-slate-400 ml-0.5">photos</span>
                        </td>
                        <td class="py-3 px-4">
                        ...
                        <td class="py-3 px-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form action="{{ route('admin.galleries.destroy', $gallery->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this gallery and all its images?')">
                                    @csrf
                                    @method('DELETE')
