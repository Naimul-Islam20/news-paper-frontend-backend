...
<div class="py-1 w-full mx-auto">
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
        ...
        {{-- Date Filter Form --}}
        <form action="{{ route('admin.statistics.visitors') }}" method="GET" class="mb-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Date From</label>
                    <input type="date" name="date_from" value="{{ $dateFrom }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-sm focus:ring-2 focus:ring-indigo-500 transition-all outline-none font-normal text-black dark:text-white">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Date To</label>
                    <input type="date" name="date_to" value="{{ $dateTo }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-sm focus:ring-2 focus:ring-indigo-500 transition-all outline-none font-normal text-black dark:text-white">
                </div>
                <div>
        ...
        <div class="p-4 bg-indigo-50 dark:bg-indigo-500/10 rounded-t-2xl border border-indigo-100 dark:border-indigo-500/20 flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <span class="text-base font-bold text-indigo-900 dark:text-indigo-200 ml-2">
            ...
            <div class="flex items-center gap-2">
                <span class="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">Total Page Views:</span>
                <span class="text-lg font-bold text-indigo-900 dark:text-indigo-200 mr-4">{{ number_format($totalPageViews) }}</span>
                <span class="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">Total Unique Visitors:</span>
                <span class="text-lg font-bold text-indigo-900 dark:text-indigo-200">{{ number_format($totalUniqueVisitors) }}</span>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                    <tr class="border-y border-slate-200 dark:border-slate-700 divide-x divide-slate-200 dark:divide-slate-700">
                        ...
                        <th class="py-3 px-4 text-[11px] font-bold text-black dark:text-slate-300 uppercase tracking-wider">Page Views</th>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="py-6 px-4 text-center text-sm text-slate-500">
